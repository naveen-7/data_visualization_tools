function varargout = beeswarm_plot_disp(varargin)
%%%% for display ONLY 
% no stats, no ttest
...
for i = 1:size(rtinp,2)
    if strcmp(usescatter,'random')
        ...
        h(i) = scatter(scat_val,rt(i,:),5,usecolor(i,1:3),'filled');                       % make scatter plot
    else
        h(i) = scatter(scat_val(i,:),rt(i,:),5,usecolor(i,1:3),'filled');
    end
    hold on
